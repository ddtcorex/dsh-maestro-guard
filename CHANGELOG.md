...
- **The full scan reads the root it was given, never the process cwd** —
  `maestro_full_scan` with an explicit `rootPath` outside the workspace
  silently scanned `process.cwd()` instead (#29).
- **The guard reads the shared settings store** — the config read still
  pointed at the retired `~/.dsh/maestro` path, so a real install saw the
  built-in defaults while the UI wrote elsewhere (#25).

### Changed

- Follow `@ddtcorex/dsh-maestro-config-lib` to `^0.3.0` and the DSH
  `0.1.7-alpha.1` peer pins (#30).
- Declare `@deepseek-ai/dsh-llm` in the test setup explicitly (#28), and
  move to vitest 3 so the suite loads the DSH 0.1.6 peers (#27).
