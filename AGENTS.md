...
- `src/host/rules.ts` — `classify()`: the rule ids (`RULE_IDS`), `DEFAULT_TIERS`, and the parsed command-surface / file-path checks that produce a `Verdict`. `secret.access` reads each parsed segment's **argv** (`isAccess`) for the path but judges the access VERB against the segment's COMMAND — a mention-only verb is never an access, an access verb inside a message/body is not the command, a heredoc body is never argv, and an `ambiguous` segment treats a protected path in its argv as an access on its own. `guard.tamper` is scoped to EDITS: a mutating verb (or `sed`/`perl` with an in-place flag) whose segment names a guard path, or a write redirection whose TARGET is one, in every spelling `pathSpellings` produces. The mutation does not have to be the segment's OWN verb: what a wrapper the parser does not unwrap runs, what a `find` action flag runs, what a `-c` script contains, and what an `xargs` takes from the segment PIPED into it all deny too (`nice rm -f <journal>`, `find . -execdir rm -f <journal> +`, `find . -exec bash -c "rm -f <journal>" +`, `echo <journal> | xargs rm -f`). A mutation WORD is only read where a command can be — the whole argv for the parser's `EXEC_WRAPPERS` ∪ `OPAQUE_VERBS` class, the tokens after a `find` action flag for a mention-led segment — so `less -p rm <journal>`, `ag rm <journal>` and `docker rm <journal>` stay reads/non-edits; the cross-segment borrow needs a real `|`, and an `xargs -I{}` whose placeholder never appears does not consume the pipe. The verb family also covers the transfers and in-place writers (`rsync`, `scp`, `curl`, `wget`, `patch`, `sponge`, `unlink`, `rmdir`, `ed`, `vi`, `vim`, `nano`); `touch` is deliberately NOT one, and an interpreter inline program or an unknown runner around a mutating verb stays open — recorded trade-offs, not oversights.
