...
- `src/host/status-tool.ts` — `createStatusTools()` / `applyStatusTools()`: the two read-only journal tools (`maestro_guard_status`, `maestro_guard_stats`), each registration its own reversible effect.
...
- `tests/*.test.ts` + `tests/*.spec.ts` — 18 vitest suites: guard, guard-handler, rules, rules-git, rules-paths, parse, parse-wrappers, decide, journal, journal-retention, migrate, redact, permission, paths, config, corpus, full-scan-tool, status-tool.
