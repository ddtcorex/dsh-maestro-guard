...
- `src/index.ts` — host `apply()`: builds the guard handler and wires it into the tool-execution waterfall (pre-execute).
- `src/approval-store.ts` — persistent approval grants (legacy migration, revoke, RMW-safe under a mutex).
- `src/secret-redactor.ts` — redacts known secret families before a call is logged/persisted.
- `src/permission-policy.ts` — policy check for whether a tool call is allowed.
- `src/augment.d.ts` — local structural types for the DSH tool-execution contract (do NOT import from `deepseek-harness`).
- `tests/*.test.ts` — vitest suites (27 tests): guard, approval-store, secret-redactor, permission.
